...
export function RetranscribeDialog({
      ...
      const unlistenComplete = await listen<RetranscriptionResult>(
        ...
        async (event) => {
          if (event.payload.meeting_id === meetingId) {
            ...
            setIsProcessing(false);
            toast.success(
              t('meetingArea.retranscribe.complete', { segments: event.payload.segments_count })
            );
  ...
  const handleStartRetranscription = async () => {
    if (!meetingFolderPath) {
      setError(t('meetingArea.retranscribe.folderMissing'));
      return;
    }
  ...
  const handleCancel = async () => {
    if (isProcessing) {
      try {
        ...
        setIsProcessing(false);
        setProgress(null);
        toast.info(t('meetingArea.retranscribe.cancelled'));
      } catch (err) {
      ...
      >
          ...
          <DialogTitle className="flex items-center gap-2">
            {isProcessing ? (
              <>
                ...
                {t('meetingArea.retranscribe.titleProcessing')}
              </>
            ) : error ? (
              <>
                <AlertCircle className="h-5 w-5 text-red-600" />
                {t('meetingArea.retranscribe.titleFailed')}
              </>
            ) : (
              <>
                <RefreshCw className="h-5 w-5 text-blue-600" />
                {t('meetingArea.retranscribe.title')}
              </>
            )}
          </DialogTitle>
          <DialogDescription>
            {isProcessing
              ? progress?.message || t('meetingArea.retranscribe.descriptionProcessing')
              : error
                ? t('meetingArea.retranscribe.descriptionFailed')
                : t('meetingArea.retranscribe.description')}
          </DialogDescription>
        </DialogHeader>
            ...
            !isParakeetModel ? (
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium">{t('meetingArea.retranscribe.language')}</span>
                </div>
                <Select value={selectedLang} onValueChange={setSelectedLang}>
                  <SelectTrigger className="w-full">
                    <SelectValue placeholder={t('meetingArea.retranscribe.selectLanguage')} />
                  </SelectTrigger>
                ...
                <p className="text-xs text-muted-foreground">
                  {t('meetingArea.retranscribe.languageHint')}
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                ...
                <p className="text-xs text-muted-foreground">
                  {t('meetingArea.retranscribe.parakeetHint')}
                </p>
              </div>
            )
          )}
          ...
          {!isProcessing && !error && availableModels.length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium">{t('meetingArea.retranscribe.model')}</span>
              </div>
              <Select value={selectedModelKey} onValueChange={setSelectedModelKey} disabled={loadingModels}>
                <SelectTrigger className="w-full">
                  <SelectValue placeholder={loadingModels ? t('meetingArea.retranscribe.loadingModels') : t('meetingArea.retranscribe.selectModel')} />
                </SelectTrigger>
                <SelectContent>
              ...
              <p className="text-xs text-muted-foreground">
                {t('meetingArea.retranscribe.modelHint')}
              </p>
            </div>
          )}
        ...
        <DialogFooter>
          {!isProcessing && !error && (
            <>
              <Button variant="outline" onClick={() => onOpenChange(false)}>
                {t('meetingArea.retranscribe.cancel')}
              </Button>
              <Button
              ...
              >
                ...
                {t('meetingArea.retranscribe.start')}
              </Button>
            </>
          )}
          {isProcessing && (
          ...
          {error && (
            <>
              <Button variant="outline" onClick={() => onOpenChange(false)}>
                {t('meetingArea.retranscribe.close')}
              </Button>
              <Button
                onClick={() => {
                  setError(null);
                ...
                variant="outline"
              >
                {t('meetingArea.retranscribe.tryAgain')}
              </Button>
            </>
          )}
        </DialogFooter>

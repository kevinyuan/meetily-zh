...
export function AudioBackendSelector({
  ...
  useEffect(() => {
    const loadBackends = async () => {
      try {
        ...
        setError(t('settingsArea.audioBackend.loadFailed'));
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleBackendChange = async (backendId: string) => {
    try {
      ...
      setError(t('settingsArea.audioBackend.changeFailed'));
    }
  };
    ...
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        <label className="text-sm font-medium text-gray-700">
          {t('settingsArea.audioBackend.title')}
        </label>
        <div className="relative">
          ...
          {showTooltip && (
            <div className="absolute z-10 left-6 top-0 w-64 p-3 text-xs bg-gray-900 text-white rounded-lg shadow-lg">
              <p className="font-semibold mb-1">{t('settingsArea.audioBackend.tooltipTitle')}</p>
              ...
              <p className="mt-2 text-gray-300">
                {t('settingsArea.audioBackend.tooltipHint')}
              </p>
            </div>
          )}
        </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  {currentBackend === backend.id && (
                    <span className="text-xs font-medium text-blue-600 bg-blue-100 px-2 py-0.5 rounded">
                      {t('settingsArea.audioBackend.active')}
                    </span>
                  )}
                  {isCoreAudio && (
                    <span className="text-xs font-medium text-gray-500 bg-gray-100 px-2 py-0.5 rounded">
                      {t('settingsArea.audioBackend.disabled')}
                    </span>
                  )}
                </div>
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>• {t('settingsArea.audioBackend.info1')}</p>
        <p>• {t('settingsArea.audioBackend.info2')}</p>
        <p>• {t('settingsArea.audioBackend.info3')}</p>
      </div>
    </div>
  );
}
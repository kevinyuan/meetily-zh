...
export const AISummary = ({ summary, status, error, onSummaryChange, onRegenerateSummary, meeting }: Props) => {
  ...
  const handleAddSection = () => {
    ...
    const newSummary: Summary = {
      ...currentSummary,
      [newSectionKey]: {
        title: t('meetingArea.summary.newSection'),
        blocks: [{
          id: newBlockId,
          type: 'text' as const,
          content: '',
  ...
  const renderErrorState = () => (
    <div className="w-full p-4 bg-red-50 border border-red-200 rounded-lg">
      <div className="flex items-center mb-2">
        ...
        <h3 className="text-red-700 font-medium">{t('meetingArea.summary.error.title')}</h3>
      </div>
      <p className="text-red-600 text-sm">{error}</p>
      <p className="text-red-500 text-xs mt-2">{t('meetingArea.summary.error.hint')}</p>
    </div>
  );
      ...
      <div className="flex items-center space-x-3">
        ...
        <div>
          <h3 className="text-blue-700 font-medium">
            {status === 'processing'
              ? t('meetingArea.summary.loading.processingTitle')
              : t('meetingArea.summary.loading.summarizingTitle')}
          </h3>
          <p className="text-blue-600 text-sm">
            {status === 'processing'
              ? t('meetingArea.summary.loading.processingDescription')
              : t('meetingArea.summary.loading.summarizingDescription')}
          </p>
        </div>
      </div>
    </div>
  ...
  if (!hasContent && status === 'completed') {
    return (
      <div className="w-full p-4 bg-gray-50 border border-gray-200 rounded-lg text-center">
        <p className="text-gray-600">{t('meetingArea.summary.empty.title')}</p>
        <p className="text-gray-500 text-sm mt-1">{t('meetingArea.summary.empty.hint')}</p>
      </div>
    );
  }
        ...
        >
          ...
          >
            ...
            <span>{selectedBlocks.length > 1
              ? t('meetingArea.blocks.copyBlocks', { num: selectedBlocks.length })
              : t('meetingArea.blocks.copyBlock')}</span>
          </button>
          <button
          ...
          >
            <span>🗑️</span>
            <span>{selectedBlocks.length > 1
              ? t('meetingArea.blocks.deleteBlocks', { num: selectedBlocks.length })
              : t('meetingArea.blocks.deleteBlock')}</span>
          </button>
        </div>
      )}

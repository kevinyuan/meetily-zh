...
export function DeviceSelection({ selectedDevices, onDeviceChange, disabled = false }: DeviceSelectionProps) {
  ...
  const fetchDevices = async () => {
    try {
      ...
      setError(t('settingsArea.devices.loadFailed'));
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const startAudioLevelMonitoring = async () => {
    try {
      ...
      if (deviceNames.length === 0) {
        setError(t('settingsArea.devices.noMicrophonesToMonitor'));
        return;
      }
      ...
      setError(t('settingsArea.devices.monitoringFailed'));
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-medium text-gray-900">{t('settingsArea.devices.title')}</h4>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <Label htmlFor="mic-selection" className="text-sm font-medium text-gray-700">
              {t('settingsArea.devices.microphone')}
            </Label>
          </div>
          <Select
          ...
          >
            <SelectTrigger id="mic-selection" className="w-full">
              <SelectValue placeholder={t('settingsArea.devices.selectMicrophone')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="default">{t('settingsArea.devices.defaultMicrophone')}</SelectItem>
          ...
          {inputDevices.length === 0 && (
            <p className="text-xs text-gray-500">{t('settingsArea.devices.noMicrophones')}</p>
          )}

          {/* Audio Level Meters for Input Devices */}
          {showLevels && inputDevices.length > 0 && (
            <div className="space-y-2 pt-2 border-t border-gray-100">
              <p className="text-xs text-gray-600 font-medium">{t('settingsArea.devices.microphoneLevels')}</p>
              {inputDevices.map((device) => {
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <Label htmlFor="system-selection" className="text-sm font-medium text-gray-700">
              {t('settingsArea.devices.systemAudio')}
            </Label>
          </div>
          ...
          >
            <SelectTrigger id="system-selection" className="w-full">
              <SelectValue placeholder={t('settingsArea.devices.selectSystemAudio')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="default">{t('settingsArea.devices.defaultSystemAudio')}</SelectItem>
          ...
          {outputDevices.length === 0 && (
            <p className="text-xs text-gray-500">{t('settingsArea.devices.noSystemAudio')}</p>
          )}
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>• <strong>{t('settingsArea.devices.info.microphoneLabel')}</strong> {t('settingsArea.devices.info.microphoneText')}</p>
        <p>• <strong>{t('settingsArea.devices.info.systemAudioLabel')}</strong> {t('settingsArea.devices.info.systemAudioText')}</p>
        {isMonitoring && (
          <p>• <strong>{t('settingsArea.devices.info.levelsLabel')}</strong> {t('settingsArea.devices.info.levelsText')}</p>
        )}
        {!isMonitoring && inputDevices.length > 0 && (
          <p>• <strong>{t('settingsArea.devices.info.tipLabel')}</strong> {t('settingsArea.devices.info.tipText')}</p>
        )}
      </div>
    </div>
  );

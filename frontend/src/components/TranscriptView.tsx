  ...
  return (
    <div className="px-4 py-2">
      ...
      {transcripts?.map((transcript, index) => {
        ...
        const displayText = originalWasEmpty && !isStreaming ? t('meetingArea.transcript.silence') : filteredText;
        ...
        const sizerText = cleanStopWords(isStreaming ? streamingTranscript.fullText : transcript.text)
          || (originalWasEmpty && !isStreaming ? t('meetingArea.transcript.silence') : '');
      ...
      {!isStopping && isRecording && !isPaused && !isProcessing && transcripts.length > 0 && (
        ...
        >
          ...
          <span className="text-sm">{t('meetingArea.transcript.listening')}</span>
        </motion.div>
      )}
          ...
          {isRecording ? (
            <>
              ...
              <p className="text-sm text-gray-600">
                {isPaused
                  ? t('meetingArea.transcript.paused')
                  : t('meetingArea.transcript.listeningForSpeech')}
              </p>
              <p className="text-xs mt-1 text-gray-400">
                {isPaused
                  ? t('meetingArea.transcript.resumeHint')
                  : t('meetingArea.transcript.speakHint')}
              </p>
            </>
          ) : (
            <>
              <p className="text-lg font-semibold">{t('meetingArea.transcript.welcome')}</p>
              <p className="text-xs mt-1">{t('meetingArea.transcript.startHint')}</p>
            </>
          )}
        </motion.div>
      )}

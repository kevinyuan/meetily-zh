  ...
  const handleBrowse = async () => {
    try {
      ...
      if (dbPath) {
        ...
        setErrorMessage(t('onboardingArea.dialogs.database.notFound'));
  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => {}}>
      <DialogContent className="sm:max-w-[600px]" onPointerDownOutside={(e) => e.preventDefault()}>
        <DialogHeader>
          <DialogTitle className="text-2xl">{t('onboardingArea.dialogs.database.title')}</DialogTitle>
          <DialogDescription className="text-base pt-2">
            {t('onboardingArea.dialogs.database.description')}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            <p className="text-sm text-gray-600">
              {t('onboardingArea.dialogs.database.browsePrompt')}
            </p>
                ...
                <>
                  ...
                  <span>
                    {importState === 'selecting'
                      ? t('onboardingArea.dialogs.database.selecting')
                      : t('onboardingArea.dialogs.database.detecting')}
                  </span>
                </>
              ) : (
                <>
                  <FolderOpen className="h-5 w-5" />
                  <span>{t('onboardingArea.dialogs.database.browse')}</span>
                </>
              )}
            </button>
          </div>
            ...
            <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-green-800">{t('onboardingArea.dialogs.database.found')}</p>
          ...
          <div className="flex flex-col gap-3 pt-2">
            ...
            >
              {importState === 'importing' ? (
                <>
                  <Loader2 className="h-5 w-5 animate-spin" />
                  <span>{t('onboardingArea.dialogs.database.importing')}</span>
                </>
              ) : importState === 'success' ? (
                <>
                  <CheckCircle2 className="h-5 w-5" />
                  <span>{t('onboardingArea.dialogs.database.success')}</span>
                </>
              ) : (
                <>
                  <Database className="h-5 w-5" />
                  <span>{t('onboardingArea.dialogs.database.importDatabase')}</span>
                </>
              )}
            </button>
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-2 bg-white text-gray-500">{t('onboardingArea.dialogs.database.or')}</span>
              </div>
            </div>
            ...
            >
              {t('onboardingArea.dialogs.database.startFresh')}
            </button>
          </div>
        </div>
      </DialogContent>

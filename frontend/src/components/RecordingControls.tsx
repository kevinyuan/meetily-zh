...
export const RecordingControls: React.FC<RecordingControlsProps> = ({
  ...
  useEffect(() => {
    const checkTauri = async () => {
      try {
        ...
        alert(t('recordingArea.alerts.initFailed'));
      }
    };
    checkTauri();
  }, [t]);
  ...
  const handleStartRecording = useCallback(async () => {
    ...
    try {
      ...
      if (errorMsg.includes('microphone') || errorMsg.includes('mic') || errorMsg.includes('input')) {
        setDeviceError({
          title: t('recordingArea.deviceErrors.microphone.title'),
          message: t('recordingArea.deviceErrors.microphone.message')
        });
      } else if (errorMsg.includes('system audio') || errorMsg.includes('speaker') || errorMsg.includes('output')) {
        setDeviceError({
          title: t('recordingArea.deviceErrors.systemAudio.title'),
          message: t('recordingArea.deviceErrors.systemAudio.message')
        });
      } else if (errorMsg.includes('permission')) {
        setDeviceError({
          title: t('recordingArea.deviceErrors.permission.title'),
          message: t('recordingArea.deviceErrors.permission.message')
        });
      } else {
        setDeviceError({
          title: t('recordingArea.deviceErrors.generic.title'),
          message: t('recordingArea.deviceErrors.generic.message')
        });
      }
    }
  ...
  const handlePauseRecording = useCallback(async () => {
    ...
    try {
      ...
      alert(t('recordingArea.alerts.pauseFailed'));
    } finally {
      setIsPausing(false);
    }
  ...
  const handleResumeRecording = useCallback(async () => {
    ...
    try {
      ...
      alert(t('recordingArea.alerts.resumeFailed'));
    } finally {
      setIsResuming(false);
    }
      ...
      <div className="flex flex-col space-y-2">
        <div className="flex items-center space-x-2 bg-white rounded-full shadow-lg px-4 py-2">
          {isProcessing && !isParentProcessing ? (
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-gray-600">{t('recordingArea.controls.processing')}</span>
            </div>
          ) : (
            <>
              {showPlayback ? (
                ...
                <>
                  {!isRecording ? (
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent>
                        <p>{t('recordingArea.controls.start')}</p>
                      </TooltipContent>
                    </Tooltip>
                  ) : (
                          ...
                          >
                            ...
                            {(isPausing || isResuming) && (
                              <div className="absolute -top-8 text-gray-600 font-medium text-xs">
                                {isPausing ? t('recordingArea.controls.pausing') : t('recordingArea.controls.resuming')}
                              </div>
                            )}
                          </button>
                        </TooltipTrigger>
                        <TooltipContent>
                          <p>{isPaused ? t('recordingArea.controls.resume') : t('recordingArea.controls.pause')}</p>
                        </TooltipContent>
                      </Tooltip>
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          ...
                          >
                            ...
                            {isStopping && (
                              <div className="absolute -top-8 text-gray-600 font-medium text-xs">
                                {t('recordingArea.controls.stopping')}
                              </div>
                            )}
                          </button>
                        </TooltipTrigger>
                        <TooltipContent>
                          <p>{t('recordingArea.controls.stop')}</p>
                        </TooltipContent>
                      </Tooltip>
                    </>
                  )}
        ...
        {isValidatingModel && (
          <div className="text-xs text-gray-600 text-center mt-2">
            {t('recordingArea.controls.validating')}
          </div>
        )}
        ...
        {deviceError && (
          <Alert variant="destructive" className="mt-4 border-red-300 bg-red-50">
            ...
            <button
              ...
              aria-label={t('recordingArea.controls.closeAlert')}
            >
              <X className="h-4 w-4" />
            </button>

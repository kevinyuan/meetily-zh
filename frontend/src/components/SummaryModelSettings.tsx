...
export function SummaryModelSettings({ refetchTrigger }: SummaryModelSettingsProps) {
  ...
  const fetchModelConfig = useCallback(async () => {
    try {
      ...
      toast.error(t('settingsArea.summary.toast.loadFailed'));
    }
  }, [t]);
    ...
    <div className='flex flex-col gap-4'>
      <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-900 mb-2">{t('settingsArea.summary.autoSummary.title')}</h3>
            <p className="text-sm text-gray-600">{t('settingsArea.summary.autoSummary.description')}</p>
          </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
        <h3 className="text-lg font-semibold mb-4">{t('settingsArea.summary.modelConfig.title')}</h3>
        <p className="text-sm text-gray-600 mb-6">
          {t('settingsArea.summary.modelConfig.description')}
        </p>

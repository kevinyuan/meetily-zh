...
const Sidebar: React.FC = () => {
  ...
  const handleDelete = async (itemId: string) => {
    ...
    try {
      ...
      // Show success toast
      toast.success(t('recordingArea.sidebar.toasts.deleteSuccess'), {
        description: t('recordingArea.sidebar.toasts.deleteSuccessDescription')
      });
      ...
      toast.error(t('recordingArea.sidebar.toasts.deleteError'), {
  ...
  const handleEditConfirm = async () => {
    ...
    if (!newTitle) {
      toast.error(t('recordingArea.sidebar.toasts.titleEmpty'));
      return;
    }
    ...
    try {
      ...
      toast.error(t('recordingArea.sidebar.toasts.titleUpdateError'), {
        description: error instanceof Error ? error.message : String(error)
      });
    }
  };
        ...
        <div className="flex flex-col items-center space-y-4 mt-4">
          ...
          <Tooltip>
            ...
            <TooltipContent side="right">
              <p>{t('recordingArea.sidebar.home')}</p>
            </TooltipContent>
          </Tooltip>
          ...
          <Tooltip>
            ...
            <TooltipContent side="right">
              <p>{isRecording ? t('recordingArea.sidebar.recordingInProgress') : t('recordingArea.sidebar.startRecording')}</p>
            </TooltipContent>
          </Tooltip>
          ...
          {betaFeatures.importAndRetranscribe && (
            <Tooltip>
              ...
              <TooltipContent side="right">
                <p>{t('recordingArea.sidebar.importAudio')}</p>
              </TooltipContent>
            </Tooltip>
          )}
          ...
          <Tooltip>
            ...
            <TooltipContent side="right">
              <p>{t('recordingArea.sidebar.meetingNotes')}</p>
            </TooltipContent>
          </Tooltip>
          ...
          <Tooltip>
            ...
            <TooltipContent side="right">
              <p>{t('recordingArea.sidebar.settings')}</p>
            </TooltipContent>
          </Tooltip>
          ...
          {item.type === 'folder' ? (
            <>
              ...
              {searchQuery && item.id === 'meetings' && isSearching && (
                <span className="ml-2 text-xs text-blue-500 animate-pulse">{t('recordingArea.sidebar.searching')}</span>
              )}
            </>
          ) : (
            <div className="flex flex-col w-full">
                ...
                {isMeetingItem && (
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                    <button
                      ...
                      aria-label={t('recordingArea.sidebar.editMeetingTitle')}
                    >
                    ...
                    <button
                      ...
                      aria-label={t('recordingArea.sidebar.deleteMeeting')}
                    >
              ...
              {hasTranscriptMatch && (
                <div className="mt-1 ml-8 text-xs text-gray-500 bg-yellow-50 p-1.5 rounded border border-yellow-100 line-clamp-2">
                  <span className="font-medium text-yellow-600">{t('recordingArea.sidebar.match')}</span> {matchingResult.matchContext}
                </div>
              )}
            </div>
          )}
  ...
  return (
    <div className="fixed top-0 left-0 h-screen z-40">
            ...
            {!isCollapsed && (
              <div className="p-3">
                ...
                <div className="relative mb-1">
                  <InputGroup >
                    <InputGroupInput placeholder={t('recordingArea.sidebar.searchPlaceholder')} value={searchQuery}
          ...
          <div className="flex-shrink-0">
            {!isCollapsed && (
              ...
              >
                ...
                <span>{t('recordingArea.sidebar.home')}</span>
              </div>
            )}
          </div>
          ...
          <div className="flex-shrink-0 p-2 border-t border-gray-100">
            ...
            >
              {isRecording ? (
                <>
                  ...
                  <span>{t('recordingArea.sidebar.recordingInProgress')}</span>
                </>
              ) : (
                <>
                  <Mic className="w-4 h-4 mr-2" />
                  <span>{t('recordingArea.sidebar.startRecording')}</span>
                </>
              )}
            </button>
            ...
            {betaFeatures.importAndRetranscribe && (
              ...
              >
                ...
                <span>{t('recordingArea.sidebar.importAudio')}</span>
              </button>
            )}
            ...
            >
              ...
              <span>{t('recordingArea.sidebar.settings')}</span>
            </button>
            <Info isCollapsed={isCollapsed} />
      ...
      <ConfirmationModal
        ...
        text={t('recordingArea.sidebar.deleteConfirm')}
      ...
      <Dialog open={editModalState.isOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="sm:max-w-[425px]">
          <VisuallyHidden>
            <DialogTitle>{t('recordingArea.sidebar.editModal.title')}</DialogTitle>
          </VisuallyHidden>
          <div className="py-4">
            <h3 className="text-lg font-semibold mb-4">{t('recordingArea.sidebar.editModal.title')}</h3>
            <div className="space-y-4">
              <div>
                <label htmlFor="meeting-title" className="block text-sm font-medium text-gray-700 mb-2">
                  {t('recordingArea.sidebar.editModal.label')}
                </label>
                <input
                  id="meeting-title"
                  type="text"
                  ...
                  placeholder={t('recordingArea.sidebar.editModal.placeholder')}
                  autoFocus
                />
              </div>
            </div>
          ...
          <DialogFooter>
            ...
            >
              {t('recordingArea.sidebar.editModal.cancel')}
            </button>
            <button
              onClick={handleEditConfirm}
              className="px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-md transition-colors"
            >
              {t('recordingArea.sidebar.editModal.save')}
            </button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

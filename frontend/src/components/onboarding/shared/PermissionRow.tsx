...
export function PermissionRow({ icon, title, description, status, isPending = false, onAction }: PermissionRowProps) {
  ...
  const getButtonText = () => {
    if (isChecking) return t('onboardingArea.permissions.row.checking');
    if (isDenied) return t('onboardingArea.permissions.row.openSettings');
    return t('onboardingArea.permissions.row.enable');
  };
          ...
          <div className="text-sm text-muted-foreground">
            {isAuthorized ? (
              <span className="text-green-600 flex items-center gap-1">
                ...
                {t('onboardingArea.permissions.row.granted')}
              </span>
            ) : isDenied ? (
              <span className="text-red-500 flex items-center gap-1">
                <XCircle className="w-3.5 h-3.5" />
                {t('onboardingArea.permissions.row.denied')}
              </span>
            ) : (

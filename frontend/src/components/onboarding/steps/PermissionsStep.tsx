...
export function PermissionsStep() {
  ...
  const handleMicrophoneAction = async () => {
    if (permissions.microphone === 'denied') {
      ...
      try {
        ...
        alert(t('onboardingArea.permissions.alerts.microphone'));
      }
      return;
    }
  ...
  const handleSystemAudioAction = async () => {
    if (permissions.systemAudio === 'denied') {
      ...
      try {
        ...
        alert(t('onboardingArea.permissions.alerts.systemAudio'));
      }
      return;
    }
  ...
  return (
    <OnboardingContainer
      title={t('onboardingArea.permissions.title')}
      description={t('onboardingArea.permissions.description')}
      step={4}
      hideProgress={true}
      ...
      <div className="max-w-lg mx-auto space-y-6">
        ...
        <div className="space-y-4">
          ...
          <PermissionRow
            ...
            title={t('onboardingArea.permissions.microphone.title')}
            description={t('onboardingArea.permissions.microphone.description')}
          ...
          <PermissionRow
            ...
            title={t('onboardingArea.permissions.systemAudio.title')}
            description={t('onboardingArea.permissions.systemAudio.description')}
        ...
        <div className="flex flex-col gap-3 pt-4">
          <Button onClick={handleFinish} disabled={!allPermissionsGranted} className="w-full h-11">
            {t('onboardingArea.permissions.finish')}
          </Button>
          ...
          >
            {t('onboardingArea.permissions.later')}
          </button>

          {!allPermissionsGranted && (
            <p className="text-xs text-center text-muted-foreground">
              {t('onboardingArea.permissions.warning')}
            </p>
          )}
        </div>
      </div>

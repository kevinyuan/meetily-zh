...
export function LanguagePickerPopover({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-label={t('settingsArea.languagePicker.dialogLabel')}
    >
      <div className="flex items-center gap-2 px-3 py-2.5 border-b border-gray-100">
        ...
        <input
          ...
          placeholder={t('settingsArea.languagePicker.searchPlaceholder')}
      ...
      <div className="max-h-80 overflow-y-auto py-1">
        {showRecents && (
          <>
            <div className="px-3 pt-1 pb-1 text-[10px] font-semibold uppercase tracking-wider text-gray-400">
              {t('settingsArea.languagePicker.recentlyUsed')}
            </div>
        ...
        {showAuto && (
          ...
          >
            <span className="flex flex-col">
              <span>{t('settingsArea.languagePicker.auto')}</span>
              {autoSubtitle && (
        ...
        {filteredAll.length > 0 && (
          <div className="px-3 pt-1 pb-1 text-[10px] font-semibold uppercase tracking-wider text-gray-400">
            {mode === "meeting"
              ? t('settingsArea.languagePicker.otherLanguages')
              : t('settingsArea.languagePicker.allLanguages')}
          </div>
        )}
        ...
        {hasNoResults && (
          <div className="px-3 py-2 text-sm text-gray-400">{t('settingsArea.languagePicker.noMatches')}</div>
        )}
      </div>
    </div>
  );

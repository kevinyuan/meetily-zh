...
const RECOMMENDED_MODEL = 'sense-voice-small-int8';
...
interface SenseVoiceModelManagerProps {
  ...
  className?: string;
  autoSave?: boolean;
  /** 'all' (or omitted) shows every model; an ISO code ('en', 'zh', ...) shows only models supporting it. */
...
export function SenseVoiceModelManager({
  ...
  useEffect(() => {
    ...
    const setupListeners = async () => {
      console.log('[SenseVoiceModelManager] Setting up event listeners...');
      ...
      unlistenProgress = await listen<{ modelName: string; progress: number }>(
        ...
        (event) => {
          ...
          if (shouldUpdate) {
            console.log(`[SenseVoiceModelManager] Progress update for ${modelName}: ${progress}%`);
    ...
    return () => {
      console.log('[SenseVoiceModelManager] Cleaning up event listeners...');
  ...
  const cancelDownload = async (modelName: string) => {
    ...
    try {
      await SenseVoiceAPI.cancelDownload(modelName);
  ...
  const deleteModel = async (modelName: string) => {
    ...
    try {
      await SenseVoiceAPI.deleteModel(modelName);
  ...
  // Only show models that support the selected language
  const visibleModels = models.filter(model => modelSupportsLanguage(model, languageFilter));

  const recommendedModel = visibleModels.find(m => m.name === RECOMMENDED_MODEL);
  const otherModels = visibleModels.filter(m => m.name !== RECOMMENDED_MODEL);

  if (visibleModels.length === 0) {
    return (
      <div className={`bg-gray-50 border border-gray-200 rounded-lg p-4 ${className}`}>
        <p className="text-sm text-gray-700">{t('modelsArea.sensevoice.noneAvailable')}</p>
        <p className="text-xs text-gray-500 mt-1">
          {languageFilter && languageFilter !== 'all'
            ? t('modelsArea.sensevoice.noneForLanguage', { language: getLanguageLabel(languageFilter) })
            : t('modelsArea.sensevoice.tryAgain')}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      {selectedModel && (
        ...
        >
          {t('modelsArea.common.usingForTranscription', { model: getModelDisplayName(selectedModel) })}
        </motion.div>
      )}
    </div>
  );
      ...
      <div className="p-4">
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1">
            ...
            {/* Supported Languages */}
            {languages.length > 0 && (
              <div className="flex flex-wrap items-center gap-1 ml-9 mt-2">
                {languages.map(language => (
                  <span
                    key={language}
                    className="bg-gray-100 text-gray-600 border border-gray-200 px-1.5 py-0.5 rounded text-[10px] font-medium"
                  >

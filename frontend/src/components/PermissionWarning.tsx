        ...
        <Alert variant="destructive" className="border-amber-400 bg-amber-50">
          ...
          <AlertTitle className="text-amber-900 font-semibold">
            <div className="flex items-center gap-2">
              ...
              {!hasMicrophone && !hasSystemAudio
                ? t('recordingArea.permissions.titleBoth')
                : !hasMicrophone
                  ? t('recordingArea.permissions.titleMicrophone')
                  : t('recordingArea.permissions.titleSystemAudio')}
            </div>
          </AlertTitle>
          {/* Action Buttons */}
          <div className="mt-4 flex flex-wrap gap-2">
            {isMacOS && !hasMicrophone && (
              ...
              >
                ...
                {t('recordingArea.permissions.openMicrophoneSettings')}
              </button>
            )}
            {isMacOS && !hasSystemAudio && (
              ...
              >
                ...
                {t('recordingArea.permissions.openScreenRecordingSettings')}
              </button>
            )}
            <button
              onClick={onRecheck}
            ...
            >
              ...
              {t('recordingArea.permissions.recheck')}
            </button>
          </div>
          <AlertDescription className="text-amber-800 mt-2">
            ...
            {!hasMicrophone && (
              <>
                <p className="mb-3">
                  {t('recordingArea.permissions.microphoneDescription')}
                </p>
                <div className="space-y-2 text-sm mb-4">
                  <p className="font-medium">{t('recordingArea.permissions.pleaseCheck')}</p>
                  <ul className="list-disc list-inside ml-2 space-y-1">
                    <li>{t('recordingArea.permissions.microphoneChecks.connected')}</li>
                    <li>{t('recordingArea.permissions.microphoneChecks.granted')}</li>
                    <li>{t('recordingArea.permissions.microphoneChecks.exclusive')}</li>
                  </ul>
                </div>
              </>
            )}
            ...
            {!hasSystemAudio && (
              <>
                <p className="mb-3">
                  {hasMicrophone
                    ? t('recordingArea.permissions.systemAudioWithMicrophone')
                    : t('recordingArea.permissions.systemAudioWithoutMicrophone')}
                </p>
                {isMacOS && (
                  <div className="space-y-2 text-sm mb-4">
                    <p className="font-medium">{t('recordingArea.permissions.macosStepsTitle')}</p>
                    <ul className="list-disc list-inside ml-2 space-y-1">
                      <li>{t('recordingArea.permissions.macosSteps.virtualDevice')}</li>
                      <li>{t('recordingArea.permissions.macosSteps.screenRecording')}</li>
                      <li>{t('recordingArea.permissions.macosSteps.routing')}</li>
                    </ul>
                  </div>
                )}
              </>

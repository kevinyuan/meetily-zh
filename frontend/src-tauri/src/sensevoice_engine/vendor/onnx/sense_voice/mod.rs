...
use crate::sensevoice_engine::vendor::decode::{ctc_greedy_decode, CtcDecoderResult, SymbolTable};
use crate::sensevoice_engine::vendor::features::{apply_cmvn, apply_lfr, compute_mel, MelConfig, WindowType};
use crate::sensevoice_engine::vendor::TranscribeError;
use crate::sensevoice_engine::vendor::{
...
impl SenseVoiceModel {
    pub fn load(model_dir: &Path, quantization: &Quantization) -> Result<Self, TranscribeError> {
        ...
        //   rc.12: `session.inputs()` (method) + `i.name()` (method).
        //   rc.10: `session.inputs` (public Vec<Input> field) + `i.name` (public String field).

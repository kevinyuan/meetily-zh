...
async fn run_import<R: Runtime>(
    ...
    let whisper_engine = if engine_kind == Engine::Whisper && total_segments > 0 {
        Some(get_or_init_whisper(&app, model.as_deref()).await?)
    } else {
        None
    };
    let parakeet_engine = if engine_kind == Engine::Parakeet && total_segments > 0 {
        Some(get_or_init_parakeet(&app, model.as_deref()).await?)
    } else {
        None
    };
    let sensevoice_engine = if engine_kind == Engine::SenseVoice && total_segments > 0 {
        ...
        Some(e) => {
            ...
            if needs_load {
                ...
                e.load_model(&target_model).await.map_err(|e| {
                    anyhow!("Failed to load SenseVoice model '{}': {}", target_model, e)
                })?;
            }
...
/// Default model for a provider type, used when the stored config points elsewhere
    ...
    match result {
        Some((provider, model)) => {
                ...
                || (provider_type == "senseVoice" && provider == "senseVoice")
            {
                ...
                Ok(default_model_for(provider_type))
            }
        }
        None => Ok(default_model_for(provider_type)),
    }
}
